...
//liste
    function listeCategories()
    {
        if (empty($this->listeCategorie)) {
            $requete = $this->pdo->prepare("SELECT id_cat,libelle from liste_cat ORDER BY libelle");
            $requete->execute();
            $resultat = array();
    ...
    {
        if (empty($this->listeComptes)) {
            $requete = $this->pdo->prepare("SELECT id,libelle from liste_comptes ORDER BY libelle");
            $requete->execute();
            $resultat = array();
            while ($result = $requete->fetch(PDO::FETCH_ASSOC)) {
                $resultat[$result['id']] = $result['libelle'];
            }
            $this->listeComptes = $resultat;
        }
    ...
    function listeOperations()
    {
        if (empty($this->listeOperations)) {
            $requete = $this->pdo->prepare("SELECT id_operations,nom_operations from operations  ORDER BY nom_operations");
            $requete->execute();
            $resultat = array();
            while ($result = $requete->fetch(PDO::FETCH_ASSOC)) {
    ...
    function listeExcel()
    {
        if (empty($this->listeExcel)) {
            $requete = $this->pdo->prepare("SELECT id_excel,libelle,position from import_excel");
            $requete->execute();
            $resultat = array();
            while ($result = $requete->fetch(PDO::FETCH_ASSOC)) {
    ...
    function listeRegroupement()
    {
        if (empty($this->listeRegroupement)) {
            $requete = $this->pdo->prepare("SELECT id_regroupement,nom from regroupement");
            $requete->execute();
            $resultat = array();
            while ($result = $requete->fetch(PDO::FETCH_ASSOC)) {
    ...
    function listeImport()
    {
        $requete = $this->pdo->prepare("SELECT r.`id`,DATE_FORMAT(r.date_crea,'%d/%m/%Y à %T') as date_crea,r.fichier, c.libelle FROM `releve` r LEFT JOIN liste_comptes c on r.compte=c.id
    ...
    function listeReleveMensuel()
    {
        $requete = $this->pdo->prepare("SELECT distinct YEAR(date) as annee_releve, MONTH( date ) AS mois_releve from releve_detail ORDER BY annee_releve DESC,mois_releve DESC");
        $requete->execute();
        $resultat = $requete->fetchall(PDO::FETCH_ASSOC);
        return $resultat;
    }
    ...
    function listeAnnee()
    {
        $requete = $this->pdo->prepare("SELECT distinct YEAR(date) as annee_releve from releve_detail ORDER BY annee_releve DESC");
        $requete->execute();
        $resultat = $requete->fetchall(PDO::FETCH_ASSOC);
        return $resultat;
    }
